...
class Model:
    ...
    def fit(self, train_payload: dict[str, np.ndarray], target_index: int, spec: dict[str, Any]) -> None:
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Direct multi-step CatBoost requires scikit-learn. Install it with `pip install scikit-learn`."
            ) from exc
        ...
        base_model = CatBoostRegressor(**kwargs)
    ...
    def predict_windows(
        ...
        if self._model is None or self._seq_len is None or self._pred_len is None:
            raise RuntimeError("CatBoost model must be fit before predict_windows")
