"""LightGBM wrapper for lag-based time-series forecasting."""
...
class Model:
    """Forecast with lightgbm.LGBMRegressor on lagged features."""
    ...
    def fit(self, train_payload: dict[str, np.ndarray], target_index: int, spec: dict[str, Any]) -> None:
        try:
            ...
            raise ModuleNotFoundError(
                "LightGBM requires `lightgbm`. Install it with `pip install lightgbm`."
            ) from exc
        ...
        try:
            ...
            raise ModuleNotFoundError(
                "Direct multi-step LightGBM requires scikit-learn. Install it with `pip install scikit-learn`."
            ) from exc
        ...
        kwargs = filter_estimator_kwargs(LGBMRegressor(), hyperparams)
    ...
    def predict_windows(
        ...
        if self._model is None or self._seq_len is None or self._pred_len is None:
            raise RuntimeError("LightGBM model must be fit before predict_windows")
